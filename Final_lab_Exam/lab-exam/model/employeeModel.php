...
function updateEmployee($id, $name, $company, $contact, $username)
{
    $conn = dbConnection();

    $query = "UPDATE employee SET name = ?, company = ?, contact = ?, username = ? WHERE id = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("ssssi", $name, $company, $contact, $username, $id);
